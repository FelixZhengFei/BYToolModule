...
public class FFSliderView: UIView, UIScrollViewDelegate {
    ...
    public func sign(index:Int, count:Int) {
        if let label = topView.viewWithTag(BaseSignTag + index) {
            let item = label as? UILabel
            item?.text = "\(count)"
            item?.isHidden = count<=0
            var w = (item?.text as NSString?)?.size(with: UIFont.systemFont(ofSize: 10)).width ?? 0.0
            if w > 10.0 {
                w = 21
            } else {
                w = 15
    ...
    private func maketopView() {
        ...
        for i in 0 ..< subViewCount {
            ...
            let size = (titles[i] as NSString).size(with: UIFont.systemFont(ofSize: titleFontSize))
            let signX = button.frame.maxX - ((btnW - size.width)/2.0)
            let signY = (btnH - size.height)/2.0 - 7.5
            let signView = UILabel(frame:CGRect(x: signX, y: signY, width: 15, height: 15))
            signView.backgroundColor = UIColor.red
            signView.textColor = UIColor.white
            signView.textAlignment = .center
            signView.font = UIFont.systemFont(ofSize: 10)
            signView.layer.cornerRadius = 7.5
            signView.clipsToBounds = true
            signView.tag = i + BaseSignTag
            signView.isHidden = true
            topView.addSubview(button)
            topView.addSubview(signView)
    ...
    private func addContentView() {
        ...
        scrollView.backgroundColor = UIColor(red: 249/255.0, green: 249/255.0, blue: 249/255.0, alpha: 1.0)
    ...
    private func changeView() {
        ...
        if let btn = topView.viewWithTag(selectedIndex + BaseTag) {
            ...
            changeSelectedBtn(btn: btn as! UIButton)
        }
    }
...
//        sliderView.transform = CGAffineTransform(translationX: xoffset, y: 0)
    ...
    private func changeSelectedBtn(btn: UIButton) {
        ...
        let offset = (itemWidth - sliderWidth)/2
        UIView.animate(withDuration: 0.25) {
            self.sliderView.frame = CGRect(x:CGFloat( index) * itemWidth  + offset, y:self.sliderView.frame.origin.y , width: self.sliderView.frame.size.width, height: self.sliderView.frame.size.height)
        }
    }
